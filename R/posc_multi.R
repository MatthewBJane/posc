posc_multi <- function(r_mat,
  ...
  beta = solve(r_mat[-outcome_idx,-outcome_idx]) %*% r_mat[outcome_idx,-outcome_idx]
  w = c(beta / sum(beta))
  r = as.numeric(w %*% r_mat[-outcome_idx,outcome_idx]) /
        as.numeric(sqrt(w %*% r_mat[-outcome_idx,-outcome_idx] %*% w))
